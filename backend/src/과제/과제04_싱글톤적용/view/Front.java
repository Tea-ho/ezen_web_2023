package 과제.과제04_싱글톤적용.view;

import java.util.Scanner;

import 과제.과제04_싱글톤적용.controller.*;
import 과제.과제04_싱글톤적용.model.*;
...
public class Front {
	...
	public void board_page() {
		while( true ) { // 무한루프 [ 종료조건 : 3 입력시 break; ] 
			...
			/*
			 * for( int j = 0 ; j<
			 * Mcontroller.getInstance().getLogSession().getBoardlist().size() ; j++ ) {
			 * Board temp = Mcontroller.getInstance().getLogSession().getBoardlist().get(j);
			 * // System.out.println( temp ); // 순환참조 걸린다. System.out.println(
			 * temp.getTitle() ); // 순환참조 안걸린다. }
			 */
